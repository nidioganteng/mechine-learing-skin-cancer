...
export default function StepHasilRF({ result }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      <div>
        <p className="text-[13px] font-semibold text-[#12283A] mb-2">{t('questionnaire.rfResult.heading')}</p>
        <h2 className="font-['Kalnia'] text-[48px] font-bold leading-none" style={{ color: warna }}>
          {isBerisiko ? t('questionnaire.rfResult.risky') : t('questionnaire.rfResult.notRisky')}
        </h2>
      </div>

      <div className="flex flex-col gap-2">
        <div className="flex justify-between text-[14px]">
          <span className="font-semibold text-[#12283A]">{t('questionnaire.rfResult.probabilityLabel')}</span>
      ...
      <div className="rounded-2xl px-5 py-4 flex gap-3" style={{ backgroundColor: '#D8E8F4' }}>
        ...
        <p className="text-[13px] text-[#12283A] leading-relaxed">
          <span className="font-semibold">{t('questionnaire.rfResult.advice')}: </span>
          {isBerisiko ? t('questionnaire.rfResult.adviceRisky') : t('questionnaire.rfResult.adviceNotRisky')}
        </p>
      </div>

      {isBerisiko && (
        <p className="text-[13px] text-[#4A6070] bg-orange-50 border border-orange-200 rounded-xl px-4 py-3"
          dangerouslySetInnerHTML={{ __html: t('questionnaire.rfResult.highRiskNote') }} />
      )}
    </div>
  )
}
