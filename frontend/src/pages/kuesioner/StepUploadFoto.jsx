...
import { CLASS_INFO, useClassInfo } from './Atoms'

function ResultCard({ result }) {
  const { t } = useTranslation()
  const info = useClassInfo(result.kelas)
  const confidencePct = Math.round(result.confidence * 100 * 10) / 10
  ...
  return (
    <div className="flex-1 flex flex-col gap-3">
      <div className="flex items-center gap-3">
        <span className="text-[16px] font-bold text-[#12283A]">{t('questionnaire.upload.resultHeading')}</span>
      ...
      <div>
        <div className="flex justify-between text-[13px] text-[#4A6070] mb-1.5">
          <span>{t('questionnaire.upload.confidence')}</span>
      ...
      <p className="text-[11px] text-gray-400 italic">{t('questionnaire.upload.disclaimer')}</p>
    </div>
  )
}
...
export default function StepUploadFoto({ onResult }) {
  ...
  async function analyze(chosen) {
    ...
    try {
      ...
      if (res.status === 'success') {
        ...
        setErrMsg(t('questionnaire.upload.errorLowQuality'))
      } else {
        setErrMsg(res.pesan || t('questionnaire.upload.errorFailed'))
      }
    } catch {
      setErrMsg(t('questionnaire.upload.errorGeneral'))
    } finally {
      setAnalyzing(false)
    }
  }
  ...
  return (
    <div className="flex flex-col gap-5">
      <div>
        <h3 className="text-[20px] font-bold text-[#12283A] mb-2">{t('questionnaire.upload.heading')}</h3>
        <p className="text-[13px] text-[#4A6070] leading-relaxed max-w-2xl">
          {t('questionnaire.upload.subtitle')}
        </p>
        <div className="w-64 h-px bg-gray-300 mt-4" />
      </div>

      <p className="text-[14px] font-bold text-[#12283A]">
        {t('questionnaire.upload.uploadLabel')}
      </p>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-[14px] font-semibold text-[#12283A]">
              {file ? file.name : t('questionnaire.upload.dragDrop')}
            </p>
            <p className="text-[12px] text-gray-500">{t('questionnaire.upload.limit')}</p>
          </div>
        </div>
        <button type="button" onClick={e => { e.stopPropagation(); fileRef.current?.click() }}
          ...
          {t('questionnaire.upload.uploadBtn')}
        </button>
      ...
      {analyzing && (
        <div className="flex items-center gap-3 text-[13px] text-[#7B9DB8]">
          ...
          {t('questionnaire.upload.analyzing')}
        </div>
      )}
      ...
      {preview && result && CLASS_INFO[result.kelas] && (
        <div className="flex flex-col sm:flex-row gap-6 mt-2">
          <div className="shrink-0 w-full sm:w-60 h-56 sm:h-60 rounded-2xl overflow-hidden bg-gray-100">
